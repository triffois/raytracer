...
Ray rotateAndOrbitRayY(Ray ray, vec3 center, float angle) {
  ...
  vec3 newDirection = vec3(
    ray.direction.x * cosAngle + ray.direction.z * sinAngle,
    ray.direction.y,
  ...
  vec3 newOrigin = vec3(
    (ray.origin.x - center.x) * cosAngle + (ray.origin.z - center.z) * sinAngle + center.x,
    ray.origin.y,
    -(ray.origin.x - center.x) * sinAngle + (ray.origin.z - center.z) * cosAngle + center.z
  );
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  ray = rotateAndOrbitRayY(ray, vec3(0, 0, 0), iTime / 10);
